    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-1">Welcome back, {owner.name}!</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="px-4 py-2 bg-slate-800 rounded-xl border border-slate-700 text-sm text-slate-300">
      ...
      {subscription && subscription.isActive ? (
        <div className="relative overflow-hidden rounded-2xl glass-card border border-primary-500/30 p-8">
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <Link to="/owner/subscription" className="px-6 py-3 bg-white text-primary-600 rounded-xl font-bold hover:bg-slate-100 transition-colors shadow-lg">
              Manage Plan
            </Link>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="glass-card p-6 rounded-2xl border border-white/10 relative overflow-hidden group">
          <div className="absolute right-0 top-0 w-32 h-32 bg-blue-500/10 rounded-full blur-2xl translate-x-10 -translate-y-10 group-hover:bg-blue-500/20 transition-colors"></div>
          <div className="relative z-10">
            <div className="flex items-center gap-3 mb-4">
              <div className="p-3 bg-blue-500/10 rounded-xl border border-blue-500/20 text-blue-400">
                <LayoutDashboard className="w-6 h-6" />
              </div>
              <span className="text-slate-400 font-medium">My Stations</span>
            </div>
            <div className="flex items-end justify-between">
              <span className="text-3xl font-bold text-white">{owner.stations?.length || 0}</span>
              <Link to="/owner/add-station" className="text-xs text-blue-400 hover:text-blue-300 flex items-center gap-1">
                + Add New
              </Link>
            </div>
          </div>
        ...
        {/* Profile Status */}
        <div className="glass-card p-6 rounded-2xl border border-white/10 relative overflow-hidden group">
          <div className="absolute right-0 top-0 w-32 h-32 bg-emerald-500/10 rounded-full blur-2xl translate-x-10 -translate-y-10 group-hover:bg-emerald-500/20 transition-colors"></div>
          <div className="relative z-10">
            <div className="flex items-center gap-3 mb-4">
            ...
            <div className="flex items-end justify-between">
              <span className="text-lg font-bold text-white">{owner.profileComplete ? 'Complete' : 'Incomplete'}</span>
        ...
        {/* Onboarding */}
        <div className="glass-card p-6 rounded-2xl border border-white/10 relative overflow-hidden group">
          <div className="absolute right-0 top-0 w-32 h-32 bg-purple-500/10 rounded-full blur-2xl translate-x-10 -translate-y-10 group-hover:bg-purple-500/20 transition-colors"></div>
          <div className="relative z-10">
            <div className="flex items-center gap-3 mb-4">
            ...
            <div className="flex items-end justify-between">
              <span className="text-3xl font-bold text-white">{owner.onboardingStep}/3</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="glass-card p-6 rounded-2xl border border-white/10">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-lg font-bold text-white flex items-center gap-2">
              <MapPin className="w-5 h-5 text-slate-400" /> My Stations
            </h2>
            <Link to="/owner/add-station" className="p-2 hover:bg-white/5 rounded-lg text-slate-400 hover:text-white transition-colors">
          ...
          {owner.stations && owner.stations.length > 0 ? (
            <div className="space-y-4">
              {owner.stations.map((station) => (
                <div key={station.id} className="flex items-center justify-between p-4 bg-slate-900/50 rounded-xl border border-slate-700/50 hover:border-slate-600 transition-colors">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400">{station.city || 'No Location'}</p>
                    </div>
                  </div>
            ...
            <div className="text-center py-12">
              ...
              <p className="text-slate-400 mb-4">No stations registered yet</p>
              <Link to="/owner/add-station" className="text-primary-400 text-sm font-medium hover:text-primary-300">
        ...
        <div className="glass-card p-6 rounded-2xl border border-white/10">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-lg font-bold text-white flex items-center gap-2">
              <BatteryCharging className="w-5 h-5 text-emerald-400" /> CNG Stock
            </h2>
            <Link to="/owner/profile" className="text-xs text-slate-400 hover:text-white transition-colors">View All</Link>
          </div>
                ...
                <div key={station.id} className={`p-4 rounded-xl border transition-all ${cngQuantity[station.id] > 0
                  ...
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      ...
                      <p className="text-xs text-slate-400">{formatLastUpdated(station.cngUpdatedAt)}</p>
                    </div>
                  ...
                  <div className="flex gap-2">
                    {editingQuantity === station.id ? (
                      ...
                      <button
                        ...
                        className="w-full bg-slate-900/50 hover:bg-slate-900 border border-slate-700 hover:border-slate-600 rounded-lg py-2 flex items-center justify-center gap-2 transition-all group"
                      >
                        <span className="font-bold text-white group-hover:text-emerald-400">{cngQuantity[station.id] || 0} kg</span>
                        <Edit2 className="w-3 h-3 text-slate-600 group-hover:text-emerald-400" />
                      </button>
                    )}
                  </div>
                </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <Link to="/owner/add-station" className="p-4 bg-slate-800/50 hover:bg-slate-800 rounded-xl border border-slate-700 flex flex-col items-center gap-3 transition-all hover:-translate-y-1 group">
            <div className="w-10 h-10 rounded-full bg-primary-500/10 flex items-center justify-center text-primary-400 group-hover:bg-primary-500 group-hover:text-white transition-colors">
              <PlusCircle className="w-5 h-5" />
            </div>
            <span className="text-sm font-medium text-slate-300 group-hover:text-white">Add Station</span>
          </Link>
          <Link to="/owner/subscription" className="p-4 bg-slate-800/50 hover:bg-slate-800 rounded-xl border border-slate-700 flex flex-col items-center gap-3 transition-all hover:-translate-y-1 group">
            ...
            <span className="text-sm font-medium text-slate-300 group-hover:text-white">Subscription</span>
          </Link>
          <Link to="/owner/profile" className="p-4 bg-slate-800/50 hover:bg-slate-800 rounded-xl border border-slate-700 flex flex-col items-center gap-3 transition-all hover:-translate-y-1 group">
          ...
          <Link to="/owner/profile" className="p-4 bg-slate-800/50 hover:bg-slate-800 rounded-xl border border-slate-700 flex flex-col items-center gap-3 transition-all hover:-translate-y-1 group">
            <div className="w-10 h-10 rounded-full bg-slate-700/50 flex items-center justify-center text-slate-400 group-hover:bg-slate-600 group-hover:text-white transition-colors">
